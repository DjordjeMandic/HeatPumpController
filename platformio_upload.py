...
# upload_URL = http://192.168.1.6/update
...
def on_upload(source, target, env):
    ...
    user = env.GetProjectOption('upload_username')
    passw = env.GetProjectOption('upload_password')
    ...
    with open(firmware_path, 'rb') as firmware:
        ...
        response = requests.post(upload_url, auth=(user, passw), data=monitor, headers={'Content-Type': monitor.content_type})
        bar.close()
